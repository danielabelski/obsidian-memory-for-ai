...
>
> **v3.1 note:** Automation is optional and non-normative; the required protocol is the Markdown toolkit in [`examples/v3-minimal-vault/`](examples/v3-minimal-vault/) plus the contract in [`SPEC-v3.md`](SPEC-v3.md). For v3 vaults, automation should write durable facts to `memory/facts/`, episodic records to `memory/events/`, generated reads to `memory/_views/`, and proposed multi-agent writes as operation envelopes via `tools/ops.py`, applied through `tools/compact.sh`.
